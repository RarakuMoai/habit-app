...
- 使用者於 2026-09-18 指定保留三個 App；版本、路徑與指令以
  [版本入口](docs/version_tracks.md) 為準。新首頁／封面／前導／故事工作使用
  `/Users/raraku/habit-app-redesign` 的 `codex/beta`；舊介面即時開發使用
  `/Users/raraku/habit-app` 的 `codex/dev`。`main` 保留基準，`gh-pages` 僅作網站輸出。
- 新任務在對應 dev／beta 分支延續，先確認工作區與未推送提交，不覆蓋其他工作。
  不自動新增分支或 worktree；確有隔離需求時先說明並取得本人同意。
  不要為了備份直接推送 main／master。
- 合併 main／master、公開發布與部署，只有使用者明確要求或確認後才做；
  自動保存版本不包含發布授權。Web 部署改為手動觸發，不隨 push 發布。
- 另經同意開出的臨時任務分支，合併且確認所有提交已在主線後才清除本機與遠端分支；
  固定 dev／beta 保留。停止開發的
