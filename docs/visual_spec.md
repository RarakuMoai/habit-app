...
快捷卡內容整組置中，同列等高；運動面盤與操作欄固定比例，超慢跑收合時開始與44pt BPM共用外框，展開時保留上方調速；點數字可精準輸入。
運動選擇列60pt高，內容視窗底部預留16pt，與玻璃分頁列分開。
工具鈕48pt、間距8pt；計時收合主操作52pt、次要操作48pt，快捷項目優先於說明與統計。

各頁依正式 parent 的收合／展開可用空間重新安排資訊，主數據與操作不得被大卡片裁掉；
`AppPressable`、`AppMotion` 及各頁持續動畫尊重降低動態。詳細尺寸與驗證狀態只在 V3.3
