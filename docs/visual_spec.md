...
等待用**一條圓角暖橘橫條**，不是轉圈。全 app 共用
[`app_waiting.dart`](../lib/widgets/app_waiting.dart) 的 `AppPageWaiting`；
顏色走 `AppWaiting` token（bar `#FF8A65`、track 白 78%）。

- **等待的燈只有一盞。** 這條橫條的來源是啟動畫面（U1 已核准），不是為各頁
  新畫的。使用者開 app 看到它，進任何一頁再看到同一條，讀起來是同一盞燈在亮。
  轉圈是「系統在忙」的通用符號，走 Material 預設形狀與 seed 派生色
  （`#8F4C37`，這個色在本文件裡不存在），跟暖色手繪世界沒有關係。
- **等待的顏色不隨時段變。** 時段色說的是「現在幾點」，等待說的是「東西還沒
  好」；疊在同一個元素上兩件事都讀不出來。
- ⚠️ **快的等待不要顯示。** 這些頁讀本機 prefs，實測多半只有一幀。無條件顯示
  等於閃一下就沒了，而一閃而過讀起來就是故障感——與「燈正在亮起來」相反。
  門檻 200ms 之後才淡入；毫秒數的真相來源是 `AppPageWaiting` 的常數。
- **淡入不是裝飾。** 硬切一樣是閃，只是慢了 200ms 才閃。Reduce Motion 拿掉
  淡入，**但門檻保留**：門檻承載的是語意（這次等待久不久），不是動畫。
- **控制項上的忙碌指示是另一件事。** 按鈕內的小 spinner 跟著按鈕前景色走，
  不換成橫條——那會變成「按鈕裡塞一條進度條」。
- 等待條要帶 `semanticsLabel`（`commonLoading`）。沒有標籤的等待對螢幕閱讀器
  就是一個沒有語意的色塊。
