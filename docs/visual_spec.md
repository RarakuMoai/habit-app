...
> 記錄既有樣式與實作限制，不是下一次視覺改版的目標。使用者指定新目標時可調整；
> 實際 token、尺寸與時間以引用的程式為準，歷史量測只適用於當時環境。
> Token 一律集中在 [`lib/utils/app_style.dart`](../lib/utils/app_style.dart)，**不要在頁面裡重新發明**。
> 全 app `Colors.grey` / 純黑歸零（`lib/dev/` 除外）。

## 獨立改版分支（2026-09-10）

`codex/experience-redesign` 目前採親子溫暖的 V3.2 方向，詳見
[V3.2：超慢跑 BPM 精簡布局](experience_redesign_v32.md)。[V3.1](experience_redesign_v31.md)、[V3](experience_redesign_v3.md)、[V2](experience_redesign_v2.md)
...
快捷卡內容整組置中，同列等高；運動面盤與操作欄固定比例，超慢跑BPM使用既有標題列的44pt膠囊，點數字可精準輸入。
工具鈕48pt、間距8pt；計時收合主操作52pt、次要操作48pt，快捷項目優先於說明與統計。

各頁依正式 parent 的收合／展開可用空間重新安排資訊，主數據與操作不得被大卡片裁掉；
`AppPressable`、`AppMotion` 及各頁持續動畫尊重降低動態。詳細尺寸與驗證狀態只在 V3.2
...
- 場景環境光、窗景、燈具與家具陰影**畫進四時段完整背景**，所以 Flutter 不再另外
  畫光束、燈暈或長影——**這是避免同一道光被打兩次，不是禁止視覺效果**。兔咪端
  仍由程式做時段白平衡（`mascotLightingForScene`）與逐房間接地影
  （`RoomLightGeometry`），那是讓角色融進已畫好的光，不是造光。
...
⚠️ **先講清楚這一節的份量：使用者實際上幾乎看不到這些畫面。** 各頁的等待讀的是
本機 prefs，實測只存在一幀，加上 200ms 門檻之後正常情況下渲染出來的是零。
唯一使用者真的會看到的等待是啟動畫面，那個歸 §啟動接縫。所以這裡記的是**寫碼
時的一致性慣例**，不是被證明過的品牌改善——不要拿它當「等待做得很好」的依據。

- 整頁／整個分頁的等待用 [`app_waiting.dart`](../lib/widgets/app_waiting.dart)
  的 `AppPageWaiting`，不要各自寫 `CircularProgressIndicator`（有測試在掃）。
  形狀沿用啟動畫面那條橫條，顏色走 `AppWaiting` token。
- 沒有 `progressIndicatorTheme` 時，裸 indicator 會吃 seed 派生的
  `colorScheme.primary`（`#8F4C37`）——那個色在本文件裡不存在。
- 等待色**不隨時段變**：時段色說的是「現在幾點」，等待說的是「東西還沒好」。
- 快的等待不顯示（門檻 200ms 後才淡入）。毫秒數看 `AppPageWaiting` 的常數。
  Reduce Motion 拿掉淡入但保留門檻。
- 控制項上的忙碌指示（按鈕內的小 spinner）跟著按鈕前景色走，不換成橫條。
- 等待條帶 `semanticsLabel`（`commonLoading`）。這條是實質修正：改版前那些裸
  spinner 對螢幕閱讀器就是一個沒有語意的色塊。
...
- 對話期間收起底部分頁，結束後恢復；使用 `Offstage` 保留主選單狀態，同時移出 hit testing 與讀屏。角色位置仍依原本房間錨點，主選單騰出的空間交給回應區。
- 選項用獨立暖白卡，沿用 `AppSurfaces.card`、`AppCardStyle.radius` 與 `AppShadows.flat`。房間底圖經透明到暖白漸層承接，不保留整片白色功能面板的上緣。
- 漸層範圍來自 `roomSceneHeight − 頂部 chrome − sceneHeight`，在背景圖結束前完全不透明，不能用固定螢幕比例掩蓋底圖接縫。
- 回應區有空間時置中，長選項與大字級可以捲動。「稍後再聊」固定在底部安全區之上；正在揭字的提示保留高度，補全文字後選項不位移。
- 面板進場沿用 280ms 輕移／淡入；Reduce Motion 取消位移、立即顯示，完整保留字幕、選項與退出入口。
