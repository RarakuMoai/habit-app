...
### 6.4 四時段完整背景驗收候選——同模擬器、同 debug 模式

2026-07-13 實測（同一台 iPhone 14 Pro Max / iOS 26.5 模擬器、
`SCENE_HOUR=13.0`、`SCENE_PERF=1`）。完整背景改成兩張相鄰 WebP crossfade，
Flutter 動態層只保留 20fps 空氣微粒、完成星光與兔咪演出：

| 情境 | 幀率 | UI avg/p95 (ms) | raster avg/p95 (ms) | 超過 16.7ms raster |
|---|---:|---:|---:|---:|
| 冷啟動首 10s（w0） | 54.9fps | 4.6 / 4.7（max 624.1） | 1.8 / 3.2 | 0 |
| 首頁活躍穩態（w1） | **59.5fps** | 2.6 / 5.2 | **2.0 / 3.2** | **0** |
| 20 秒閒置門檻所在視窗（w2） | 1.9fps | 6.9 / 17.0 | 2.3 / 3.6 | 0 |
| 閒置完成（w3 起） | **0fps** | – | – | 0 |

相較 §6.1 舊首頁活躍基準，raster avg 7.3→2.0ms（約 −73%）、
raster p95 15.0→3.2ms（約 −79%）、超預算 raster 幀 21→0；閒置後仍為
0fps。debug 數據只供同機相對比較，實機耗電／溫度仍依 §6.3 補測。

四張 1122×1402 原始 PNG 共 8,869,308 bytes；驗收候選 WebP 共
1,213,438 bytes，打包體積減少約 86.3%。逐張縮減為：清晨 85.1%、
白天 88.3%、黃昏 85.1%、夜晚 86.8%；以原尺寸與實際首頁畫面檢查，未見
會影響 CG 線條、漸層或文字可讀性的壓縮瑕疵。
