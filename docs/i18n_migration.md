...
| 英文文案 | **全部頁面逐句細讀完畢**（2026-07-29）。剩下的是母語人士 proofreading |
...
- 破折號用 `—`（em dash），前後留空格
- 分隔號一律用 `·`，不要用 `|`（中文那邊的全形｜是中文自己的事）
- 不用 Oxford comma（`start, finish or come back`）。唯一例外是列表中的某一
  項本身含 `and`，不加逗號會讀錯的時候
- 拼字用美式（`license`、`colors`），不要混英式
- 不用分號；兩件事就寫兩句，或用 em dash

### 單複數：英文的 `1 days` 是硬傷

**只要文案裡的數字有可能等於 1，英文就必須用 ICU plural。** 中文沒有這個
問題，所以直譯過來的字串幾乎都缺這一層。寫法：

```json
"roundsCount": "{count, plural, =1{1 round} other{{count} rounds}}"
```

- **zh 那邊維持單一形式就好**，不用跟著寫 plural 語法。gen-l10n 是逐語言解析
  訊息的，只有英文用 plural 完全合法（前提：`@key` 的 placeholder 型別是
  `int`／`num`，本來就都是）。
- 檢查方法：找 `{x} 名詞s` 這種樣式，再回去看那個數字的下限。
  （例：桌遊玩家最少 2 位 → 不用 plural；專注回合最少 1 → 一定要。）
- **句型擋不住 plural 時要改句型**：像「{count} {unit} · …」這種把單位當參數
  傳進來的組法，英文的複數在單位字上，ICU 救不了——把它拆成兩條完整句子
  （見 `rvFocusCountLine` / `rvExerciseCountLine`）。或者把數字挪到名詞後面
  （`ties {t}`），問題就消失了。
- 步進器那種「數字一行、單位另一行」的版面不算，單位維持不變形。
...
**逐句細讀已全部做完**（2026-07-29）：首頁、喝水、體重、設定（前一批）＋
計時、家庭、衣櫃、回顧／報到、小蛇、前導頁、夥伴檔案（這一批），最後再做了
一次全檔單複數與標點掃描。全域問題（術語、大小寫、撞字、分號、Please）更早
之前就處理過。規則見上一節。

**加新文案時照上一節的規則寫**；要再做一輪細讀的話，做法是：

1. 按頁 dump 該批的「中文 │ 英文 │ description」並排
   （`grep` ARB 太零碎，直接用小 script 從 dart 檔抓 `l10n.xxx` 再對 ARB）
2. 逐條讀，挑這幾類問題：
   - 技術詞漏進使用者文案（default、reading、entry 之類）
   - 直譯後不自然（"a cup of 250 ml"、"Not now" 暗示之後會再問）
   - 與其他 key 前後不一致（同一個按鈕在說明裡被寫成別的字）
   - 單複數（見上一節，最容易漏也最刺眼）
   - **英文指到的 UI 真的存在嗎**（曾出現「press start」但那顆是圖示鈕、
     「Settings → Features」但頁名其實是 Feature toggles）
   - **共用 key 在兩邊語意一樣嗎**（中文的「天」在名片是相處天數、在報到頁
     是連勝天數，英文得拆成兩個 key）
...
——英文母語不會這樣寫月份（應該是 `July 2026`）。要改有兩條路，都超出「只動
英文」的範圍，所以留著：

- 補 12 個月份名 key（翻譯者友善，但 ARB 多 12 條）
- 改用 `DateFormat.yMMMM(localeName)`（乾淨，但中文會變成「2026年7月」，
  少掉現在數字前後的空格）

同一頁的週標籤是程式直接組的 `7/23 – 7/29`（兩種語言共用），要一起想。
...
1. ~~英文語氣打磨~~——**已完成**（2026-07-29 全部頁面逐句細讀完畢）。
   剩下的是上架前找母語人士 proofreading，以及月份標籤那項（見上面）。
...
做完 2 才能拿掉 `main.dart` 的 locale 鎖定。
