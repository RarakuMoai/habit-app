...
2026-09-14 更新：封面已獲實裝授權並由 [V4 原生動態封面](entry_cover_v4.md) 接續，
版本為 `1.0.1+2026091403`；封面曲依後續指示改用既有 `matsurinohi`。
後續本人選定新的 941×1672 Image 2.5 兔咪底圖，並核可接入封面與短初見；
葉片 UV 變形已改為輪廓不變、手機可辨識的整層剛體風推動作，詳見同一封面紀錄。
`2026091403` signed iOS Release 已建立並通過獨立 codesign 與內嵌 V6 素材核對，
尚未安裝實體裝置、合併主線或公開發布。
以下 V3 素材與 `2026091201` 驗證是前輪歷史，
其首次／回訪／PIN／備份分流仍沿用，不把歷史截圖當成 V4 驗證結果。
