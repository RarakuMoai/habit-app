...
- ⚠️ **待機生命感目前卡在兩張缺圖**（2026-08-04 查證，兩件都已釘進程式註解）：
  ...
  以上是現有架構的限制與當時提出的素材方案，不是所有未來做法都被禁止。
  若任務涉及改待機模型，先向使用者確認目標，再檢查 baseline／進度關係與
  現有眨眼資產；不能為了眨眼讓完成狀態在演出收尾時遺失。
...
功能 UI 已導入 ARB；實際 key 以 `lib/l10n/*.arb` 為準，不在文件鎖定數量。
下面是已知刻意保留的中文字串類別，不代表日後新增的中文字串都已審核。
改之前先確認是顯示文字、儲存值或識別鍵。
...
`widgets/mascot_bubbles.dart` 的 `assert` 訊息、註解與正則字元類別
