...
| 當前階段 | **沒有進行中的 milestone。** `polish/quiet-wait` 已降級為一致性修補批次（見下）。下一步是使用者跑一輪實機驗收，把累積的五項一次清掉 |
...
- `main.dart` 與 `home_page.dart` 各有一個裸 `CircularProgressIndicator` fallback。
  ...
  **2026-08-05 修正：這句話是錯的。** `ThemeData` 沒有 `progressIndicatorTheme`，
  ...
  `visual_spec.md` 裡根本不存在。詳見下方「等待狀態」。
...
### 等待狀態（一致性修補批次；曾被誤開成 milestone U3）

Branch `polish/quiet-wait`。**2026-08-05 使用者當場推翻它的 milestone 資格，
已降級。** 程式保留，文件份量收掉，不辦 Creative Proof，不動用
`/code-review ultra`。

**為什麼被推翻——這一段比程式本身值得留：**

使用者的話是「這個等待只是當出現特殊狀況、起碼不要空白所填空用的，根本是不值得
去處理的地方」。他是對的：

- 這 16 處是 `!_loaded` 的 fallback 分支，讀的是本機 prefs，實測只存在一幀。
  **加上 200ms 門檻之後，正常情況下渲染出來的東西是零。**
- 使用者真的會看到的等待只有啟動畫面，那個 U1 已經處理完並驗收過。這一輪對它
  沒有任何改善，只是把它抽成共用元件。
- 所以照 §5.2 的排序標準「可察覺程度 × 改善幅度 × 成本／風險」，可察覺程度
  ≈ 0，乘積就是 0。**它在選題階段就該被刷掉。**

**總控的兩處判斷失誤，寫下來避免重犯：**

1. **選題時用錯了尺。** 當時的理由是「候選池五個裡唯一現在能純程式推進的」——
   那是在挑做得動的，不是在挑值得做的。§5.2 那把尺當時就在文件裡，沒拿出來用。
2. **中途發現不可見時，反應是錯的。** 測試跑出「只存在第一幀」的那一刻，是停下來
   說「這不夠格當 milestone」的訊號。實際做的卻是加門檻讓它更不可見，再包裝成
   勝利並送出 A/B 對照圖。那張圖沒有造假，但它拍的是一個實際上不會出現的畫面
   ——**證據成立、證據對應的情境不存在**，跟 §7 記的 half 錨點錯誤同類。

**還站得住、所以程式留著的部分：**

- 等待條補了 `semanticsLabel`。改版前那些裸 spinner 對螢幕閱讀器就是一個沒有
  語意的色塊，這是實質修正。
- 16 份重複收成一個共用元件，加原始碼掃描測試擋住之後再長出第十七種。
- `progressIndicatorTheme` 讓漏網的 indicator 不再吃 seed 派生的 `#8F4C37`。

這些是管線衛生，照本文件對 `polish/tab-weight` 的分類就是**一致性修補批次**：
用技術 gate 審過即可合併，不需要 Creative Proof。

⬜ 若之後真要問「有沒有頁面超過 200ms」，那是實機題。**目前沒有任何證據顯示
存在這樣的頁面**——門檻是為一個未經驗證的情境設計的。
...
- **等待長什麼樣子這件事，這個 app 已經設計過了**：`_StartupLoadingBar`
  ...
  它只被用過一次，就在啟動畫面。
- ⚠️ **這份只讀審查漏掉了唯一重要的問題：這些畫面出不出得來。** 掃到的都是
  「長什麼樣」，沒有一項在問「使用者看得到嗎」。答案（只存在一幀）是實作到
  一半跑測試才撞出來的。下次只讀審查對任何 fallback 分支要先問可達性與停留
  時間，再談它長什麼樣。
