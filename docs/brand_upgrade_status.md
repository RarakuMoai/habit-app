...
**狀態：** branch `polish/quiet-wait`（`0a85355`，3 個 commit）。
2026-08-05 使用者裁決 **KEEP**；Bible extraction 已完成（`visual_spec.md` §等待）。
**技術 gate 未過，不得 merge**——已指定由使用者手動觸發 `/code-review ultra`。
合併後 main 應為 814 + 3 = 817 項。

實作分兩層。視覺層：把啟動畫面那條已核准的橫條抽成 `AppLoadingBar`／
`AppPageWaiting`，啟動改用同一個，16 個房間跟著用；顏色進 `AppWaiting` token，
補 `progressIndicatorTheme` 讓漏網的 indicator 至少是暖橘。行為層：**門檻 200ms
之後才淡入**。

門檻那一層是實作到一半才長出來的，理由值得記住：**測試逼出「等待狀態只存在
第一幀」這個事實**（斷言必須卡在 `pumpWidget` 之後、下一個 `pump` 之前）。
沒有門檻的話，這個 milestone 只是把一個閃一下的棕圈圈換成一個閃一下的橘條，
而一閃而過讀起來正好就是故障感。這是「測試證明數字對，證明不了看得到」的
第二個實例——第一個是 idle-life 的待機姿。

⬜ **待實機**：門檻 200ms 在真機上是否選得對——也就是實際有多少頁會越過它。
模擬器與 widget test 都證明不了真實載入時間。若實機發現幾乎沒有頁面越過門檻，
那不是門檻錯，是「這個 app 根本沒有慢到需要等待畫面」——那會是好消息，
但也表示 U3 的可見價值主要在啟動那一段。
