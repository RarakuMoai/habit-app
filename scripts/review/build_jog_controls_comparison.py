#!/usr/bin/env python3
"""Export the native BPM review without changing the V3.1 reference and V3.2 baseline."""
import hashlib
import html
import json
...
OUTPUT = ROOT / 'design_trials/experience_redesign/review_v33'
...
def main():
    ...
    (OUTPUT / 'capture-manifest.json').write_text(json.dumps({
        'baseline': 'fb0dfef', 'visual_reference': '6d79c63', 'branch': 'codex/experience-redesign',
    ...
    pairs = [
        ...
        ('after/exercise-jog-compact.webp', 'V3.3'),
    ]
    ...
    alignment = ''.join(figure(f'after/exercise-{kind}-compact.webp', label)
                        for kind, label in [('tabata', 'Tabata'), ('hiit', 'HIIT'), ('emom', 'EMOM'), ('gym', '重訓'), ('jog', '超慢跑')])
    states = ''.join(figure(f'after/{name}.webp', title) for name, title in [
        ...
        ('exercise-jog-expanded', '展開：保留上方調速'),
        ('jog-expanded-editor', '展開的精準輸入'),
        ('jog-expanded-paused', '展開暫停'),
    ])
    page = '''<!doctype html><html lang="zh-Hant"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>兔咪 V3.3・收合控制與底部留白</title><style>
...
</style><body><header><p>兔咪新體驗 / V3.3</p><h1>回到熟悉的收合分區，<br>讓按鈕與留白更舒服。</h1><p>依照偏好的 V3.1，保留左面盤、右側集中操作。開始與 BPM 改為上下相接的一組控制，五個運動選項則收成柔和的選擇列，與玻璃分頁之間保留固定留白。</p><p><a href="#comparison">修正前後</a> · <a href="#alignment">五種運動的留白</a> · <a href="#controls">操作與狀態</a></p></header><main>
<section id="comparison"><h2>超慢跑，修正前後</h2><p>V3.3 保留 V3.1 的 BPM 位置，以共用外框減少零碎框線與重複間距。開始 52pt、BPM 44pt、重設與跳過 48pt，操作功能與觸控大小完整保留。</p><div class="compare">COMPARISONS</div></section>
<section id="alignment"><h2>運動選擇列，與底部分頁分開</h2><p>五個選項改成 60pt 的一組選擇列，只凸顯選中項目。內容視窗底部固定保留 16pt，玻璃列本身另有 6pt 上緣，所以實際兩層表面相隔 22pt。</p><div class="gallery">ALIGNMENT</div></section>
